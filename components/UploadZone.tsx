...
export default function UploadZone({ mode: initialMode }: { mode: Mode }) {
 ...
 img.src = url;
 if (img.complete) {
 img.naturalWidth > 0 ? resolve() : reject(new Error("Failed to load image"));
 }
 });
 setProgress(20);
 await sleep(100);
 ...
 <p className="text-gray-500 animate-step-in inline-flex items-center gap-2">
 ...
 <div className="h-1 rounded-full overflow-hidden bg-gray-200/50">
 <div
 ...
 <p className="text-xs text-amber-500/80 mt-4 flex items-center justify-center gap-1.5">
...
// ---- RESULT VIEW ----
 ...
 <p className="text-sm text-gray-500">Reading first video frame...</p>
 </div>
 )}
 ...
 Play the video to find watermarks, pause, then brush over them. Mask applies to every frame.
 </p>
 )}
 ...
 return (
 <div className="max-w-2xl mx-auto animate-fade-in">
 {/* Mode toggle — only for background */}
 {mode === "background" && !fileURL && (
 <div className="flex items-center justify-center gap-3 mb-6">
 <span className={`text-sm font-semibold ${!autoMode ? "text-indigo-600" : "text-gray-400"}`}>Manual</span>
 <button
 className={`relative w-12 h-6 rounded-full transition-colors ${autoMode ? "bg-indigo-500" : "bg-gray-300"}`}
 onClick={() => setAutoMode(!autoMode)}
 aria-label="Toggle processing mode"
 >
 <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${autoMode ? "translate-x-6" : "translate-x-0.5"}`} />
 </button>
 <span className={`text-sm font-semibold ${autoMode ? "text-indigo-600" : "text-gray-400"}`}>Auto</span>
 </div>
 )}

 {/* File type tabs */}
 <div className="flex gap-1 justify-center mb-6 p-1 rounded-xl bg-gray-100 max-w-xs mx-auto">
 {mode === "background" ? (
